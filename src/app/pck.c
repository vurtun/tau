...
static const struct file_group_def file_groups[] = {
  [FILE_GRP_OTHER]      = {.name = "Other",         .icon = RES_ICO_FILE,         .files = file_unkown_defs,  .cnt = cntof(file_unkown_defs)},
  [FILE_GRP_DIR]        = {.name = "Folder",        .icon = RES_ICO_FOLDER_OPEN,  .files = file_folder_defs,  .cnt = cntof(file_folder_defs)},
  [FILE_GRP_TXT]        = {.name = "Text",          .icon = RES_ICO_FILE_TXT,     .files = file_text_defs,    .cnt = cntof(file_text_defs)},
  [FILE_GRP_PROG]       = {.name = "Programming",   .icon = RES_ICO_FILE_CODE,    .files = file_prog_defs,    .cnt = cntof(file_prog_defs)},
  [FILE_GRP_AUDIO]      = {.name = "Audio",         .icon = RES_ICO_FILE_AUDIO,   .files = file_audio_defs,   .cnt = cntof(file_audio_defs)},
  [FILE_GRP_VIDEO]      = {.name = "Video",         .icon = RES_ICO_FILE_VIDEO,   .files = file_video_defs,   .cnt = cntof(file_video_defs)},
  [FILE_GRP_IMAGE]      = {.name = "Image",         .icon = RES_ICO_FILE_IMG,     .files = file_images_defs,  .cnt = cntof(file_images_defs)},
  [FILE_GRP_FONT]       = {.name = "Font",          .icon = RES_ICO_FILE,         .files = file_font_defs,    .cnt = cntof(file_font_defs)},
  [FILE_GRP_3D]         = {.name = "3DModel",       .icon = RES_ICO_FILE,         .files = file_3d_defs,      .cnt = cntof(file_3d_defs)},
  [FILE_GRP_EXEC]       = {.name = "Executable",    .icon = RES_ICO_FILE,         .files = file_exe_defs,     .cnt = cntof(file_exe_defs)},
  [FILE_GRP_ARCHIVE]    = {.name = "Archive",       .icon = RES_ICO_FILE_ARCHIVE, .files = file_archive_defs, .cnt = cntof(file_archive_defs)},
};
// clang-format on
...
static const struct file_tbl_col_def file_tbl_def[FILE_TBL_MAX] = {
  [FILE_TBL_NAME] =  {.title = strv("Name"),           .sort = {file_cmp_asc,      file_cmp_desc},       .ui = {.type = GUI_LAY_SLOT_DYN, .size =   1, .con = {200, 800}}},
  [FILE_TBL_TYPE] =  {.title = strv("Type"),           .sort = {file_cmp_type_asc, file_cmp_type_desc},  .ui = {.type = GUI_LAY_SLOT_FIX, .size = 100, .con = {100, 800}}},
  [FILE_TBL_SIZE] =  {.title = strv("Size"),           .sort = {file_cmp_size_asc, file_cmp_size_desc},  .ui = {.type = GUI_LAY_SLOT_FIX, .size = 120, .con = {100, 800}}},
  [FILE_TBL_PERM] =  {.title = strv("Permission"),     .sort = {file_cmp_perm_asc, file_cmp_perm_desc},  .ui = {.type = GUI_LAY_SLOT_FIX, .size = 160, .con = {100, 800}}},
  [FILE_TBL_DATE] =  {.title = strv("Date Modified"),  .sort = {file_cmp_time_asc, file_cmp_time_desc},  .ui = {.type = GUI_LAY_SLOT_FIX, .size = 300, .con = {200, 800}}},
};
static const struct gui_split_lay_slot file_split_def[FILE_SPLIT_MAX] = {
  {.type = GUI_LAY_SLOT_FIX, .size = 400, .con = {100, 800}},
...
file_type(struct str ext) {
  ...
  for arr_eachv(grp, file_groups) {
    const struct file_def *def = 0;
    for arr_each(def, grp->files, grp->cnt) {
...
static void
file_lst_view_clr(struct file_list_view *lst, struct sys *_sys) {
  assert(lst);
  assert(_sys);

  dyn_free(lst->fltr, _sys);
  dyn_free(lst->elms, _sys);
  dyn_free(lst->full_path, _sys);
  dyn_free(lst->fnd_buf, _sys);
  arena_free(&lst->mem, _sys);

  zero2(lst->off);
  lst->fltr = arena_dyn(&lst->mem, _sys, unsigned long, 128);
  lst->elms = arena_dyn(&lst->mem, _sys, struct file_elm, 256);
  lst->nav_path = arena_dyn(&lst->mem, _sys, char, 256);
  lst->full_path = arena_dyn(&lst->mem, _sys, char, 256);
  lst->fnd_buf = arena_dyn(&lst->mem, _sys, char, MAX_FILTER);
...
static void
file_lst_view_add_path(struct file_list_view *lst, struct sys *_sys,
                       struct str path) {
  assert(lst);
  assert(_sys);

  struct file_elm elm = {0};
  struct arena_scope s = {0};
  arena_scope_push(&s, &lst->mem);
  ...
  if (!_sys->file.info(_sys, &info, elm.fullpath, &lst->mem)) {
    arena_scope_pop(&s, &lst->mem, _sys);
    return;
  }
  elm.size = info.siz;
  ...
  case SYS_FILE_DEF: {
    ...
    elm.ext = arena_str(&lst->mem, _sys, ext);
  ...
  /* add new file info */
  dyn_add(lst->elms, _sys, elm);
  dyn_fit(lst->fltr, _sys, bits_to_long(dyn_cnt(lst->elms)));
...
file_view_cd(struct file_view *fs, struct sys *_sys, struct str path) {
  ...
  struct arena *tmp = fs->tmp_arena;
  confine arena_scope(tmp, &scp, _sys) {
    struct str sys_path = arena_str(tmp, _sys, path);
    file_lst_view_clr(&fs->lst, _sys);
    dyn_asn_str(fs->lst.full_path, _sys, sys_path);
    dyn_asn_str(fs->lst.nav_path, _sys, sys_path);

    /* add all files in directory */
    struct sys_dir_iter it = {0};
    for sys_dir_lst_each(_sys, &it, tmp, sys_path) {
...
static void
file_list_view_init(struct file_view *fs, struct sys *_sys, struct gui_ctx *ctx) {
  assert(fs);
  assert(_sys);
  assert(ctx);

  fs->lst.elms = arena_dyn(&fs->lst.mem, _sys, struct file_elm, 1024);
  fs->lst.fltr = arena_dyn(&fs->lst.mem, _sys, unsigned long, 128);
  fs->lst.nav_path = arena_dyn(&fs->lst.mem, _sys, char, 1024);
  fs->lst.full_path = arena_dyn(&fs->lst.mem, _sys, char, 1024);
  fs->lst.fnd_buf = arena_dyn(&fs->lst.mem, _sys, char, MAX_FILTER);
...
file_view_init(struct sys *_sys, struct gui_ctx *ctx, struct arena *mem,
                struct arena *tmp_arena) {
  assert(_sys);
  assert(ctx);
  assert(mem);
  ...
  struct file_view *fs = arena_obj(mem, _sys, struct file_view);
  fs->lst_rev = 1;
  fs->tree_rev = 1;
  fs->tmp_arena = tmp_arena;
  fs->home = arena_str(mem, _sys, str0(getenv("HOME")));
  ...
  file_list_view_init(fs, _sys, ctx);
...
ui_file_view_tbl_elm(struct gui_ctx *ctx, struct gui_tbl *tbl,
  ...
  gui.tbl.lst.elm.begin(ctx, tbl, elm, elm_id, is_sel);
  {
    ...
    confine gui_cfg_pushu_on_scope(&gui,stk,&ctx->cfg.col[GUI_COL_ICO], dir_col, fi->isdir) {
      ...
      gui.tbl.lst.elm.col.txt_ico(ctx, tbl, lay, elm, fi->name, fi->ico);
      gui.tbl.lst.elm.col.txt(ctx, tbl, lay, elm, str0(fi->type->name), 0);
      gui.tbl.lst.elm.col.txtf(ctx, tbl, lay, elm, &algn, "%zu", fi->size);
      gui.tbl.lst.elm.col.txt(ctx, tbl, lay, elm, str0(fi->perms), 0);
...
ui_file_view_tree_node(struct gui_ctx *ctx, struct gui_tree_node *node,
  ...
  {
    struct gui_cfg_stk stk[1] = {0};
    confine gui_cfg_pushu_scope(&gui, stk, &ctx->cfg.col[GUI_COL_ICO], col_rgb_hex(0xeecd4a)) {
