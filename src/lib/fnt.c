...
typedef int fnt__test_oversample_pow2[(FNT_MAX_OVERSAMPLE & (FNT_MAX_OVERSAMPLE-1)) == 0 ? 1 : -1];
...
#define FNT_ifloor(x)   ((int) math_floori(x))
#define FNT_iceil(x)    ((int) math_ceili(x))
#define FNT_sqrt(x)      math_sqrt(x)
#define FNT_pow(x,y)     math_pow(x,y)
#define FNT_fmod(x,y)    math_mod(x,y)
#define FNT_cos(x)       math_cos(x)
#define FNT_acos(x)      math_acos(x)
...
#ifndef FNT_memcpy
#include <string.h>
#define FNT_memcpy       mcpy
#define FNT_memset       mset
#endif
...
static fnt_uint8 fnt__buf_get8(fnt__buf *b)
{
   if (b->cursor >= b->size)
      return 0;
...
static fnt_uint8 fnt__buf_peek8(fnt__buf *b)
{
   if (b->cursor >= b->size)
      return 0;
   return b->data[b->cursor];
...
static void fnt__buf_seek(fnt__buf *b, int o)
{
   FNT_assert(!(o > b->size || o < 0));
   b->cursor = (o > b->size || o < 0) ? b->size : o;
}

static void fnt__buf_skip(fnt__buf *b, int o)
{
   fnt__buf_seek(b, b->cursor + o);
}

static fnt_uint32 fnt__buf_get(fnt__buf *b, int n)
{
   fnt_uint32 v = 0;
   int i;
   FNT_assert(n >= 1 && n <= 4);
...
static fnt__buf fnt__new_buf(const void *p, size_t size)
{
   fnt__buf r;
...
fnt__dict_get(fnt__buf *b, int key) {
   ...
   while (b->cursor < b->size) {
      ...
      op = fnt__buf_get8(b);
      if (op == 12)  op = fnt__buf_get8(b) | 0x100;
      if (op == key) return fnt__buf_range(b, start, end-start);
   }
...
#define fnt_tag4(p,c0,c1,c2,c3) ((p)[0] == (c0) && (p)[1] == (c1) && (p)[2] == (c2) && (p)[3] == (c3))
#define fnt_tag(p,str)           fnt_tag4(p,str[0],str[1],str[2],str[3])

static int fnt__isfont(fnt_uint8 *font)
{
   // check the version number
   if (fnt_tag4(font, '1',0,0,0))  return 1; // TrueType 1
   if (fnt_tag(font, "typ1"))   return 1; // TrueType with type 1 font -- we don't support this!
   if (fnt_tag(font, "OTTO"))   return 1; // OpenType with CFF
   if (fnt_tag4(font, 0,1,0,0)) return 1; // OpenType 1.0
   if (fnt_tag(font, "true"))   return 1; // Apple specification for TrueType fonts
   return 0;
}

// @OPTIMIZE: binary search
static fnt_uint32 fnt__find_table(fnt_uint8 *data, fnt_uint32 fontstart, const char *tag)
{
...
static int fnt_GetFontOffsetForIndex_internal(unsigned char *font_collection, int index)
{
...
static int fnt_GetNumberOfFonts_internal(unsigned char *font_collection)
{
...
static fnt__buf fnt__get_subrs(fnt__buf cff, fnt__buf fontdict)
{
...
static int fnt__get_svg(fnt_fontinfo *info)
{
   fnt_uint32 t;
   if (info->svg < 0) {
      t = fnt__find_table(info->data, info->fontstart, "SVG ");
      if (t) {
         fnt_uint32 offset = ttULONG(info->data + t + 2);
         info->svg = t + offset;
      } else {
         info->svg = 0;
      }
...
static int fnt_InitFont_internal(fnt_fontinfo *info, unsigned char *data, int fontstart)
{
   fnt_uint32 cmap, t;
...
static int fnt_FindGlyphIndex(const fnt_fontinfo *info, int unicode_codepoint)
{
   ...
   if (format == 0) { // apple byte encoding
      ...
      if ((fnt_uint32) unicode_codepoint >= first && (fnt_uint32) unicode_codepoint < first+count)
...

      {
         ...
         if (offset == 0)
            return (fnt_uint16) (unicode_codepoint + ttSHORT(data + index_map + 14 + segcount*4 + 2 + 2*item));
      ...
      fnt_int32 low,high;
      low = 0; high = (fnt_int32)ngroups;
      // Binary search the right group.
      while (low < high) {
         fnt_int32 mid = low + ((high-low) >> 1); // rounds down, so low <= mid < high
...
{
   return fnt_GetGlyphShape(info, fnt_FindGlyphIndex(info, unicode_codepoint), vertices);
}

static void fnt_setvertex(fnt_vertex *v, fnt_uint8 type, fnt_int32 x, fnt_int32 y, fnt_int32 cx, fnt_int32 cy)
{
   v->type = type;
   v->x = (fnt_int16) x;
   v->y = (fnt_int16) y;
...
static int fnt__GetGlyfOffset(const fnt_fontinfo *info, int glyph_index)
{
   int g1,g2;
...
static int fnt__GetGlyphInfoT2(const fnt_fontinfo *info, int glyph_index, int *x0, int *y0, int *x1, int *y1);

static int fnt_GetGlyphBox(const fnt_fontinfo *info, int glyph_index, int *x0, int *y0, int *x1, int *y1)
{
   if (info->cff.size) {
      fnt__GetGlyphInfoT2(info, glyph_index, x0, y0, x1, y1);
   } else {
      int g = fnt__GetGlyfOffset(info, glyph_index);
      if (g < 0) return 0;
...
{
   return fnt_GetGlyphBox(info, fnt_FindGlyphIndex(info,codepoint), x0,y0,x1,y1);
}
...
static int fnt_IsGlyphEmpty(const fnt_fontinfo *info, int glyph_index)
{
   ...
   int g;
   if (info->cff.size)
      return fnt__GetGlyphInfoT2(info, glyph_index, NULL, NULL, NULL, NULL) == 0;
   g = fnt__GetGlyfOffset(info, glyph_index);
   if (g < 0) return 1;
   numberOfContours = ttSHORT(info->data + g);
   return numberOfContours == 0;
}

static int fnt__close_shape(fnt_vertex *vertices, int num_vertices, int was_off, int start_off,
    fnt_int32 sx, fnt_int32 sy, fnt_int32 scx, fnt_int32 scy, fnt_int32 cx, fnt_int32 cy)
{
   if (start_off) {
      if (was_off)
         fnt_setvertex(&vertices[num_vertices++], FNT_vcurve, (cx+scx)>>1, (cy+scy)>>1, cx,cy);
      fnt_setvertex(&vertices[num_vertices++], FNT_vcurve, sx,sy,scx,scy);
   } else {
      if (was_off)
         fnt_setvertex(&vertices[num_vertices++], FNT_vcurve,sx,sy,cx,cy);
      else
         fnt_setvertex(&vertices[num_vertices++], FNT_vline,sx,sy,0,0);
   }
   return num_vertices;
}

static int fnt__GetGlyphShapeTT(const fnt_fontinfo *info, int glyph_index, fnt_vertex **pvertices)
{
   fnt_int16 numberOfContours;
   fnt_uint8 *endPtsOfContours;
   fnt_uint8 *data = info->data;
   fnt_vertex *vertices=0;
   int num_vertices=0;
   int g = fnt__GetGlyfOffset(info, glyph_index);
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      vertices = (fnt_vertex *) FNT_malloc(m * sizeof(vertices[0]), info->userdata);
      if (vertices == 0)
         return 0;
      ...
      for (i=0; i < n; ++i) {
         ...
         if (next_move == i) {
            if (i != 0)
               num_vertices = fnt__close_shape(vertices, num_vertices, was_off, start_off, sx,sy,scx,scy,cx,cy);
            ...
            fnt_setvertex(&vertices[num_vertices++], FNT_vmove,sx,sy,0,0);
            was_off = 0;
            ...
            if (!(flags & 1)) { // if it's a curve
               if (was_off) // two off-curve control points in a row means interpolate an on-curve midpoint
                  fnt_setvertex(&vertices[num_vertices++], FNT_vcurve, (cx+x)>>1, (cy+y)>>1, cx, cy);
               cx = x;
               cy = y;
               was_off = 1;
            } else {
               if (was_off)
                  fnt_setvertex(&vertices[num_vertices++], FNT_vcurve, x,y, cx, cy);
               else
                  fnt_setvertex(&vertices[num_vertices++], FNT_vline, x,y,0,0);
               was_off = 0;
            }
         }
      }
      ...
      while (more) {
         ...
         // Get indexed glyph.
         comp_num_verts = fnt_GetGlyphShape(info, gidx, &comp_verts);
         if (comp_num_verts > 0) {
            // Transform vertices.
            for (i = 0; i < comp_num_verts; ++i) {
               fnt_vertex* v = &comp_verts[i];
               fnt_vertex_type x,y;
               x=v->x; y=v->y;
            ...
            // Append vertices.
            tmp = (fnt_vertex*)FNT_malloc((num_vertices+comp_num_verts)*sizeof(fnt_vertex), info->userdata);
            if (!tmp) {
               if (vertices) FNT_free(vertices, info->userdata);
               if (comp_verts) FNT_free(comp_verts, info->userdata);
               return 0;
            }
            if (num_vertices > 0 && vertices) FNT_memcpy(tmp, vertices, num_vertices*sizeof(fnt_vertex));
            FNT_memcpy(tmp+num_vertices, comp_verts, comp_num_verts*sizeof(fnt_vertex));
            if (vertices) FNT_free(vertices, info->userdata);
            vertices = tmp;
...
typedef struct
{
   ...
   fnt_vertex *pvertices;
   int num_vertices;
} fnt__csctx;

#define FNT__CSCTX_INIT(bounds) {bounds,0, 0,0, 0,0, 0,0,0,0, NULL, 0}

static void fnt__track_vertex(fnt__csctx *c, fnt_int32 x, fnt_int32 y)
{
...
static void fnt__csctx_v(fnt__csctx *c, fnt_uint8 type, fnt_int32 x, fnt_int32 y, fnt_int32 cx, fnt_int32 cy, fnt_int32 cx1, fnt_int32 cy1)
{
   if (c->bounds) {
      ...
      fnt_setvertex(&c->pvertices[c->num_vertices], type, x, y, cx, cy);
...
static void fnt__csctx_close_shape(fnt__csctx *ctx)
{
   if (ctx->first_x != ctx->x || ctx->first_y != ctx->y)
      fnt__csctx_v(ctx, FNT_vline, (int)ctx->first_x, (int)ctx->first_y, 0, 0, 0, 0);
}

static void fnt__csctx_rmove_to(fnt__csctx *ctx, float dx, float dy)
{
...
static void fnt__csctx_rline_to(fnt__csctx *ctx, float dx, float dy)
{
   ctx->x += dx;
   ctx->y += dy;
...
static void fnt__csctx_rccurve_to(fnt__csctx *ctx, float dx1, float dy1, float dx2, float dy2, float dx3, float dy3)
{
   float cx1 = ctx->x + dx1;
   float cy1 = ctx->y + dy1;
   float cx2 = cx1 + dx2;
   ...
   ctx->x = cx2 + dx3;
   ctx->y = cy2 + dy3;
   fnt__csctx_v(ctx, FNT_vcubic, (int)ctx->x, (int)ctx->y, (int)cx1, (int)cy1, (int)cx2, (int)cy2);
}

static fnt__buf fnt__get_subr(fnt__buf idx, int n)
{
...
static fnt__buf fnt__cid_get_glyph_subrs(const fnt_fontinfo *info, int glyph_index)
{
   fnt__buf fdselect = info->fdselect;
   ...
   return fnt__get_subrs(info->cff, fnt__cff_index_get(info->fontdicts, fdselector));
}

static int fnt__run_charstring(const fnt_fontinfo *info, int glyph_index, fnt__csctx *c)
{
   int in_header = 1, maskbits = 0, subr_stack_height = 0, sp = 0, v, i, b0;
   int has_subrs = 0, clear_stack;
   float s[48];
   fnt__buf subr_stack[10], subrs = info->subrs, b;
   float f;
...
#define FNT__CSERR(s) (0)
   ...
   while (b.cursor < b.size) {
      ...
      case 0x1E: // vhcurveto
         ...
         for (;;) {
            if (i + 3 >= sp) break;
            fnt__csctx_rccurve_to(c, 0, s[i], s[i+1], s[i+2], s[i+3], (sp - i == 5) ? s[i + 4] : 0.0f);
            i += 4;
      hvcurveto:
            if (i + 3 >= sp) break;
            fnt__csctx_rccurve_to(c, s[i], 0, s[i+1], s[i+2], (sp - i == 5) ? s[i+4] : 0.0f, s[i+3]);
            i += 4;
         }
         break;
      ...
      case 0x08: // rrcurveto
         ...
         for (; i + 5 < sp; i += 6)
            fnt__csctx_rccurve_to(c, s[i], s[i+1], s[i+2], s[i+3], s[i+4], s[i+5]);
         break;
      ...
      case 0x1B: // hhcurveto
         ...
         for (; i + 3 < sp; i += 4) {
            if (b0 == 0x1B)
               fnt__csctx_rccurve_to(c, s[i], f, s[i+1], s[i+2], s[i+3], 0.0);
            else
               fnt__csctx_rccurve_to(c, f, s[i], s[i+1], s[i+2], 0.0, s[i+3]);
            f = 0.0;
         }
         break;
...
static int fnt__GetGlyphShapeT2(const fnt_fontinfo *info, int glyph_index, fnt_vertex **pvertices)
{
   // runs the charstring twice, once to count and once to output (to avoid realloc)
   fnt__csctx count_ctx = FNT__CSCTX_INIT(1);
   fnt__csctx output_ctx = FNT__CSCTX_INIT(0);
   if (fnt__run_charstring(info, glyph_index, &count_ctx)) {
      *pvertices = (fnt_vertex*)FNT_malloc(count_ctx.num_vertices*sizeof(fnt_vertex), info->userdata);
...
static int fnt__GetGlyphInfoT2(const fnt_fontinfo *info, int glyph_index, int *x0, int *y0, int *x1, int *y1)
{
   fnt__csctx c = FNT__CSCTX_INIT(1);
...
static int fnt_GetGlyphShape(const fnt_fontinfo *info, int glyph_index, fnt_vertex **pvertices)
{
   if (!info->cff.size)
      return fnt__GetGlyphShapeTT(info, glyph_index, pvertices);
   else
      return fnt__GetGlyphShapeT2(info, glyph_index, pvertices);
}
...
static int fnt__GetGlyphKernInfoAdvance(const fnt_fontinfo *info, int glyph1, int glyph2)
{
   fnt_uint8 *data = info->data + info->kern;
...
static fnt_int32 fnt__GetCoverageIndex(fnt_uint8 *coverageTable, int glyph)
{
...
static fnt_int32  fnt__GetGlyphClass(fnt_uint8 *classDefTable, int glyph)
{
   ...
   switch (classDefFormat)
   {
      case 1: {
         ...
         if (glyph >= startGlyphID && glyph < startGlyphID + glyphCount)
            return (fnt_int32)ttUSHORT(classDef1ValueArray + 2 * (glyph - startGlyphID));
         break;
      }
...
static fnt_int32 fnt__GetGlyphGPOSInfoAdvance(const fnt_fontinfo *info, int glyph1, int glyph2)
{
   ...
   for (i=0; i<lookupCount; ++i) {
      ...
      for (sti=0; sti<subTableCount; sti++) {
         ...
         fnt_int32 coverageIndex = fnt__GetCoverageIndex(table + coverageOffset, glyph1);
         ...
         switch (posFormat) {
            case 1: {
               ...
               if (valueFormat1 == 4 && valueFormat2 == 0) { // Support more formats?
                  ...
                  fnt_uint16 pairPosOffset = ttUSHORT(table + 10 + 2 * coverageIndex);
            ...
            case 2: {
               ...
               if (valueFormat1 == 4 && valueFormat2 == 0) { // Support more formats?
                  ...
                  int glyph1class = fnt__GetGlyphClass(table + classDef1Offset, glyph1);
                  int glyph2class = fnt__GetGlyphClass(table + classDef2Offset, glyph2);

                  fnt_uint16 class1Count = ttUSHORT(table + 12);
                  fnt_uint16 class2Count = ttUSHORT(table + 14);
                  fnt_uint8 *class1Records, *class2Records;
                  fnt_int16 xAdvance;
...
{
   fnt_GetGlyphBitmapBoxSubpixel(font, glyph, scale_x, scale_y,0.0f,0.0f, ix0, iy0, ix1, iy1);
}
...
{
   fnt_GetGlyphBitmapBoxSubpixel(font, fnt_FindGlyphIndex(font,codepoint), scale_x, scale_y,shift_x,shift_y, ix0,iy0,ix1,iy1);
}
...
{
   fnt_GetCodepointBitmapBoxSubpixel(font, codepoint, scale_x, scale_y,0.0f,0.0f, ix0,iy0,ix1,iy1);
}
...
typedef struct fnt__hheap_chunk
{
   struct fnt__hheap_chunk *next;
} fnt__hheap_chunk;

typedef struct fnt__hheap
{
...
static void *fnt__hheap_alloc(fnt__hheap *hh, size_t size, void *userdata)
{
   if (hh->first_free) {
      ...
      if (hh->num_remaining_in_head_chunk == 0) {
         ...
         fnt__hheap_chunk *c = (fnt__hheap_chunk *) FNT_malloc(sizeof(fnt__hheap_chunk) + size * count, userdata);
         if (c == NULL)
            return NULL;
         c->next = hh->head;
         hh->head = c;
      ...
      return (char *) (hh->head) + sizeof(fnt__hheap_chunk) + size * hh->num_remaining_in_head_chunk;
   }
}

static void fnt__hheap_free(fnt__hheap *hh, void *p)
{
   *(void **) p = hh->first_free;
   hh->first_free = p;
}

static void fnt__hheap_cleanup(fnt__hheap *hh, void *userdata)
{
   fnt__hheap_chunk *c = hh->head;
   while (c) {
      fnt__hheap_chunk *n = c->next;
      FNT_free(c, userdata);
      c = n;
   }
}

typedef struct fnt__edge {
   float x0,y0, x1,y1;
   int invert;
} fnt__edge;


typedef struct fnt__active_edge
{
...
static fnt__active_edge *fnt__new_active(fnt__hheap *hh, fnt__edge *e, int off_x, float start_point, void *userdata)
{
   fnt__active_edge *z = (fnt__active_edge *) fnt__hheap_alloc(hh, sizeof(*z), userdata);
   ...
   z->x = FNT_ifloor(FNT_FIX * e->x0 + z->dx * (start_point - e->y0)); // use z->dx so when we offset later it's by the same amount
   z->x -= off_x * FNT_FIX;
...
static void fnt__fill_active_edges(unsigned char *scanline, int len, fnt__active_edge *e, int max_weight)
{
         ...
         if (w == 0) {
            ...
            if (i < len && j >= 0) {
               if (i == j) {
                  ...
                  scanline[i] = scanline[i] + (fnt_uint8) ((x1 - x0) * max_weight >> FNT_FIXSHIFT);
               } else {
                  if (i >= 0) // add antialiasing for x0
                     scanline[i] = scanline[i] + (fnt_uint8) (((FNT_FIX - (x0 & FNT_FIXMASK)) * max_weight) >> FNT_FIXSHIFT);
                  else
                     i = -1; // clip

                  if (j < len) // add antialiasing for x1
                     scanline[j] = scanline[j] + (fnt_uint8) (((x1 & FNT_FIXMASK) * max_weight) >> FNT_FIXSHIFT);
                  else
                     j = len; // clip
...
static void fnt__rasterize_sorted_edges(fnt__bitmap *result, fnt__edge *e, int n, int vsubsample, int off_x, int off_y, void *userdata)
{
   fnt__hheap hh = { 0, 0, 0 };
   fnt__active_edge *active = NULL;
   int y,j=0;
   ...
   while (j < result->h) {
      ...
      for (s=0; s < vsubsample; ++s) {
         ...
         while (e->y0 <= scan_y) {
            if (e->y1 > scan_y) {
               fnt__active_edge *z = fnt__new_active(&hh, e, off_x, scan_y, userdata);
               if (z != NULL) {
...
static void fnt__handle_clipped_edge(float *scanline, int x, fnt__active_edge *e, float x0, float y0, float x1, float y1)
{
   if (y0 == y1) return;
   FNT_assert(y0 < y1);
...
static float fnt__sized_trapezoid_area(float height, float top_width, float bottom_width)
{
   FNT_assert(top_width >= 0);
   FNT_assert(bottom_width >= 0);
   return (top_width + bottom_width) / 2.0f * height;
}

static float fnt__position_trapezoid_area(float height, float tx0, float tx1, float bx0, float bx1)
{
   return fnt__sized_trapezoid_area(height, tx1 - tx0, bx1 - bx0);
}

static float fnt__sized_triangle_area(float height, float width)
{
   return height * width / 2;
}

static void fnt__fill_active_edges_new(float *scanline, float *scanline_fill, int len, fnt__active_edge *e, float y_top)
{
   ...
   while (e) {
      ...
      if (e->fdx == 0) {
         ...
         if (x0 < len) {
            if (x0 >= 0) {
               fnt__handle_clipped_edge(scanline,(int) x0,e, x0,y_top, x0,y_bottom);
               fnt__handle_clipped_edge(scanline_fill-1,(int) x0+1,e, x0,y_top, x0,y_bottom);
            } else {
               fnt__handle_clipped_edge(scanline_fill-1,0,e, x0,y_top, x0,y_bottom);
            }
         }
      } else {
         float x0 = e->fx;
         ...
         if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
            ...
            if ((int) x_top == (int) x_bottom) {
               ...
               scanline[x]      += fnt__position_trapezoid_area(height, x_top, x+1.0f, x_bottom, x+1.0f);
               ...
               scanline[x1] += fnt__sized_triangle_area(area, x1+1 - x_top);
               ...
               FNT_assert(FNT_fabs(area) <= 1.01f); // accumulated error from area += step unless we round step down
               FNT_assert(sy1 > y_final-0.01f);

               // area covered in the last pixel is the rectangle from all the pixels to the left,
               // plus the trapezoid filled by the line segment in this pixel all the way to the right edge
               scanline[x2] += area + sign * fnt__position_trapezoid_area(sy1-y_final, (float) x2, x2+1.0f, x_bottom, x2+1.0f);
...
static void fnt__rasterize_sorted_edges(fnt__bitmap *result, fnt__edge *e, int n, int vsubsample, int off_x, int off_y, void *userdata)
{
   ...
   if (result->w > 64)
      scanline = (float *) FNT_malloc((result->w*2+1) * sizeof(float), userdata);
   else
      scanline = scanline_data;
   ...
   while (j < result->h) {
      ...
      while (e->y0 <= scan_y_bottom) {
         if (e->y0 != e->y1) {
            fnt__active_edge *z = fnt__new_active(&hh, e, off_x, scan_y_top, userdata);
            if (z != NULL) {
               if (j == 0 && off_y != 0) {
                  if (z->ey < scan_y_top) {
               ...
               FNT_assert(z->ey >= scan_y_top); // if we get really unlucky a tiny bit of an edge can be out of bounds
               // insert at front
               z->next = active;
               active = z;
            }
      ...
      if (active)
         fnt__fill_active_edges_new(scanline, scanline2+1, result->w, active, scan_y_top);
...
static void fnt__sort_edges_ins_sort(fnt__edge *p, int n)
{
   int i,j;
   for (i=1; i < n; ++i) {
      fnt__edge t = p[i], *a = &t;
...
static void fnt__sort_edges_quicksort(fnt__edge *p, int n)
{
...
static void fnt__sort_edges(fnt__edge *p, int n)
{
...
static void fnt__rasterize(fnt__bitmap *result, fnt__point *pts, int *wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void *userdata)
{
   ...
   e = (fnt__edge *) FNT_malloc(sizeof(*e) * (n+1), userdata); // add an extra one as a sentinel
   if (e == 0) return;
   n = 0;
...
static void fnt__add_point(fnt__point *points, int n, float x, float y)
{
...
static int fnt__tesselate_curve(fnt__point *points, int *num_points, float x0, float y0, float x1, float y1, float x2, float y2, float objspace_flatness_squared, int n)
{
   // midpoint
   ...
   if (dx*dx+dy*dy > objspace_flatness_squared) { // half-pixel error allowed... need to be smaller if AA
      fnt__tesselate_curve(points, num_points, x0,y0, (x0+x1)/2.0f,(y0+y1)/2.0f, mx,my, objspace_flatness_squared,n+1);
      fnt__tesselate_curve(points, num_points, mx,my, (x1+x2)/2.0f,(y1+y2)/2.0f, x2,y2, objspace_flatness_squared,n+1);
   } else {
...
static void fnt__tesselate_cubic(fnt__point *points, int *num_points, float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
{
   ...
   float dx = x3-x0;
   float dy = y3-y0;
   float longlen = (float) (FNT_sqrt(dx0*dx0+dy0*dy0)+FNT_sqrt(dx1*dx1+dy1*dy1)+FNT_sqrt(dx2*dx2+dy2*dy2));
   ...
   if (flatness_squared > objspace_flatness_squared) {
      ...
      fnt__tesselate_cubic(points, num_points, x0,y0, x01,y01, xa,ya, mx,my, objspace_flatness_squared,n+1);
      fnt__tesselate_cubic(points, num_points, mx,my, xb,yb, x23,y23, x3,y3, objspace_flatness_squared,n+1);
   } else {
...
static fnt__point *fnt_FlattenCurves(fnt_vertex *vertices, int num_verts, float objspace_flatness, int **contour_lengths, int *num_contours, void *userdata)
{
   fnt__point *points=0;
   int num_points=0;
   ...
   *contour_lengths = (int *) FNT_malloc(sizeof(**contour_lengths) * n, userdata);
   ...
   for (pass=0; pass < 2; ++pass) {
      float x=0,y=0;
      if (pass == 1) {
         points = (fnt__point *) FNT_malloc(num_points * sizeof(points[0]), userdata);
      ...
      for (i=0; i < num_verts; ++i) {
         switch (vertices[i].type) {
            ...
            case FNT_vline:
               ...
               break;
            case FNT_vcurve:
               fnt__tesselate_curve(points, &num_points, x,y,
...
static void fnt_Rasterize(fnt__bitmap *result, float flatness_in_pixels, fnt_vertex *vertices, int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, int invert, void *userdata)
{
   float scale            = scale_x > scale_y ? scale_y : scale_x;
   int winding_count      = 0;
   int *winding_lengths   = NULL;
   fnt__point *windings = fnt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, &winding_lengths, &winding_count, userdata);
   if (windings) {
      fnt__rasterize(result, windings, winding_lengths, winding_count, scale_x, scale_y, shift_x, shift_y, x_off, y_off, invert, userdata);
...
static unsigned char *fnt_GetGlyphBitmapSubpixel(const fnt_fontinfo *info, float scale_x, float scale_y, float shift_x, float shift_y, int glyph, int *width, int *height, int *xoff, int *yoff)
{
   ...
   fnt__bitmap gbm;
   fnt_vertex *vertices;
   int num_verts = fnt_GetGlyphShape(info, glyph, &vertices);
   ...
   fnt_GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, shift_x, shift_y, &ix0,&iy0,&ix1,&iy1);
   ...
   if (gbm.w && gbm.h) {
      ...
      if (gbm.pixels) {
         ...
         fnt_Rasterize(&gbm, 0.35f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ix0, iy0, 1, info->userdata);
      }
   }
   FNT_free(vertices, info->userdata);
   return gbm.pixels;
...
{
   return fnt_GetGlyphBitmapSubpixel(info, scale_x, scale_y, 0.0f, 0.0f, glyph, width, height, xoff, yoff);
}
...
static void fnt_MakeGlyphBitmapSubpixel(const fnt_fontinfo *info, unsigned char *output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph)
{
   ...
   fnt_GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, shift_x, shift_y, &ix0,&iy0,0,0);
   gbm.pixels = output;
   gbm.w = out_w;
   gbm.h = out_h;
...
{
   fnt_MakeGlyphBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, 0.0f,0.0f, glyph);
}
...
{
   return fnt_GetGlyphBitmapSubpixel(info, scale_x, scale_y,shift_x,shift_y, fnt_FindGlyphIndex(info,codepoint), width,height,xoff,yoff);
}
...
{
   fnt_MakeGlyphBitmapSubpixelPrefilter(info, output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, oversample_x, oversample_y, sub_x, sub_y, fnt_FindGlyphIndex(info,codepoint));
}
...
{
   fnt_MakeGlyphBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, fnt_FindGlyphIndex(info,codepoint));
}
...
{
   return fnt_GetCodepointBitmapSubpixel(info, scale_x, scale_y, 0.0f,0.0f, codepoint, width,height,xoff,yoff);
}
...
{
   fnt_MakeCodepointBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, 0.0f,0.0f, codepoint);
}
...
static int fnt_BakeFontBitmap_internal(unsigned char *data, int offset,  // font location (use offset=0 for plain .ttf)
                                float pixel_height,                     // height of font in pixels
                                unsigned char *pixels, int pw, int ph,  // bitmap to be filled in
                                int first_char, int num_chars,          // characters to bake
                                fnt_bakedchar *chardata)
{
   float scale;
   int x,y,bottom_y, i;
   fnt_fontinfo f;
   f.userdata = NULL;
   if (!fnt_InitFont(&f, data, offset))
      return -1;
   FNT_memset(pixels, 0, pw*ph); // background of 0 around pixels
   x=y=1;
   bottom_y = 1;

   scale = fnt_ScaleForPixelHeight(&f, pixel_height);

   for (i=0; i < num_chars; ++i) {
      int advance, lsb, x0,y0,x1,y1,gw,gh;
      int g = fnt_FindGlyphIndex(&f, first_char + i);
      fnt_GetGlyphHMetrics(&f, g, &advance, &lsb);
      ...
      FNT_assert(x+gw < pw);
      FNT_assert(y+gh < ph);
      fnt_MakeGlyphBitmap(&f, pixels+x+y*pw, gw,gh,pw, scale,scale, g);
...
static void fnt_GetBakedQuad(const fnt_bakedchar *chardata, int pw, int ph, int char_index, float *xpos, float *ypos, fnt_aligned_quad *q, int opengl_fillrule)
{
   float d3d_bias = opengl_fillrule ? 0 : -0.5f;
   float ipw = 1.0f / pw, iph = 1.0f / ph;
   const fnt_bakedchar *b = chardata + char_index;
...
static void rpq_init_target(rpq_context *con, int pw, int ph, rpq_node *nodes, int num_nodes)
{
   con->width  = pw;
   con->height = ph;
   con->x = 0;
...
static void rpq_pack_rects(rpq_context *con, rpq_rect *rects, int num_rects)
{
   int i;
...
static int fnt_PackBegin(fnt_pack_context *spc, unsigned char *pixels, int pw, int ph, int stride_in_bytes, int padding, void *alloc_context)
{
   rpq_context *context = (rpq_context *) FNT_malloc(sizeof(*context)            ,alloc_context);
   int            num_nodes = pw - padding;
   rpq_node    *nodes   = (rpq_node    *) FNT_malloc(sizeof(*nodes  ) * num_nodes,alloc_context);
   ...
   rpq_init_target(context, pw-padding, ph-padding, nodes, num_nodes);
...
static void fnt_PackSetOversampling(fnt_pack_context *spc, unsigned int h_oversample, unsigned int v_oversample)
{
   ...
   if (h_oversample <= FNT_MAX_OVERSAMPLE)
      spc->h_oversample = h_oversample;
   if (v_oversample <= FNT_MAX_OVERSAMPLE)
...
static void fnt__h_prefilter(unsigned char *pixels, int w, int h, int stride_in_bytes, unsigned int kernel_width)
{
   unsigned char buffer[FNT_MAX_OVERSAMPLE];
   int safe_w = w - kernel_width;
   int j;
   FNT_memset(buffer, 0, FNT_MAX_OVERSAMPLE); // suppress bogus warning from VS2013 -analyze
   for (j=0; j < h; ++j) {
      int i;
      unsigned int total;
...
static void fnt__v_prefilter(unsigned char *pixels, int w, int h, int stride_in_bytes, unsigned int kernel_width)
{
   unsigned char buffer[FNT_MAX_OVERSAMPLE];
      ...
      switch (kernel_width) {
         case 2:
            for (i=0; i <= safe_h; ++i) {
               ...
               buffer[(i+kernel_width) & FNT__OVER_MASK] = pixels[i*stride_in_bytes];
...
static float fnt__oversample_shift(int oversample)
{
   if (!oversample)
      return 0.0f;
...
static int fnt_PackFontRangesGatherRects(fnt_pack_context *spc, const fnt_fontinfo *info, fnt_pack_range *ranges, int num_ranges, rpq_rect *rects)
{
   ...
   for (i=0; i < num_ranges; ++i) {
      ...
      float scale = fh > 0 ? fnt_ScaleForPixelHeight(info, fh) : fnt_ScaleForMappingEmToPixels(info, -fh);
      ranges[i].h_oversample = (unsigned char) spc->h_oversample;
      ranges[i].v_oversample = (unsigned char) spc->v_oversample;
      for (j=0; j < ranges[i].num_chars; ++j) {
         int x0,y0,x1,y1;
         int codepoint = ranges[i].array_of_unicode_codepoints == NULL ? ranges[i].first_unicode_codepoint_in_range + j : ranges[i].array_of_unicode_codepoints[j];
         int glyph = fnt_FindGlyphIndex(info, codepoint);
         if (glyph == 0 && (spc->skip_missing || missing_glyph_added)) {
            rects[k].w = rects[k].h = 0;
         } else {
            fnt_GetGlyphBitmapBoxSubpixel(info,glyph,
                                            scale * spc->h_oversample,
                                            scale * spc->v_oversample,
                                            0,0,
                                            &x0,&y0,&x1,&y1);
            rects[k].w = (rpq_coord) (x1-x0 + spc->padding + spc->h_oversample-1);
            rects[k].h = (rpq_coord) (y1-y0 + spc->padding + spc->v_oversample-1);
            if (glyph == 0)
   ...
   for (i=0; i < num_ranges; ++i) {
      ...
      for (j=0; j < ranges[i].num_chars; ++j) {
         ...
         if (r->was_packed && r->w != 0 && r->h != 0) {
            fnt_packedchar *bc = &ranges[i].chardata_for_range[j];
            int advance, lsb, x0,y0,x1,y1;
            int codepoint = ranges[i].array_of_unicode_codepoints == NULL ? ranges[i].first_unicode_codepoint_in_range + j : ranges[i].array_of_unicode_codepoints[j];
            int glyph = fnt_FindGlyphIndex(info, codepoint);
            rpq_coord pad = (rpq_coord) spc->padding;
            ...
            fnt_GetGlyphHMetrics(info, glyph, &advance, &lsb);
            fnt_GetGlyphBitmapBox(info, glyph,
                                    scale * spc->h_oversample,
                                    scale * spc->v_oversample,
                                    &x0,&y0,&x1,&y1);

            int unscaled_ascent, unscaled_descent, unscaled_line_gap;
            fnt_GetFontVMetrics(info, &unscaled_ascent, &unscaled_descent, &unscaled_line_gap);
            const float ascent = math_floor(unscaled_ascent * scale + 0.5f);
            ...
            bc->yoff     =       (float)  (y0 * recip_v + sub_y) + ascent;
...
fnt_PackFontRanges(fnt_pack_context *spc, const unsigned char *fontdata,
   ...
   rects = (rpq_rect *) FNT_malloc(sizeof(*rects) * n, spc->user_allocator_context);
   if (rects == NULL)
      return 0;

   info.userdata = spc->user_allocator_context;
   fnt_InitFont(&info, fontdata, fnt_GetFontOffsetForIndex(fontdata,font_index));

   n = fnt_PackFontRangesGatherRects(spc, &info, ranges, num_ranges, rects);

   fnt_PackFontRangesPackRects(spc, rects, n);

   return_value = fnt_PackFontRangesRenderIntoRects(spc, &info, ranges, num_ranges, rects);
...
static int fnt_PackFontRange(fnt_pack_context *spc, const unsigned char *fontdata, int font_index, float font_size,
            int first_unicode_codepoint_in_range, int num_chars_in_range, fnt_packedchar *chardata_for_range)
{
   fnt_pack_range range;
   range.first_unicode_codepoint_in_range = first_unicode_codepoint_in_range;
   range.array_of_unicode_codepoints = NULL;
   range.num_chars                   = num_chars_in_range;
   range.chardata_for_range          = chardata_for_range;
   range.font_size                   = font_size;
   return fnt_PackFontRanges(spc, fontdata, font_index, &range, 1);
}
...
static void fnt_GetScaledFontVMetrics(const unsigned char *fontdata, int index, float size, float *ascent, float *descent, float *lineGap)
{
   ...
   float scale;
   fnt_fontinfo info;
   fnt_InitFont(&info, fontdata, fnt_GetFontOffsetForIndex(fontdata, index));
   scale = size > 0 ? fnt_ScaleForPixelHeight(&info, size) : fnt_ScaleForMappingEmToPixels(&info, -size);
   fnt_GetFontVMetrics(&info, &i_ascent, &i_descent, &i_lineGap);
   *ascent  = (float) i_ascent  * scale;
   *descent = (float) i_descent * scale;
   *lineGap = (float) i_lineGap * scale;
}

static void fnt_GetPackedQuad(const fnt_packedchar *chardata, int pw, int ph, int char_index, float *xpos, float *ypos, fnt_aligned_quad *q, int align_to_integer)
{
   float ipw = 1.0f / pw, iph = 1.0f / ph;
   const fnt_packedchar *b = chardata + char_index;

   if (align_to_integer) {
      float x = (float) FNT_ifloor((*xpos + b->xoff) + 0.5f);
      float y = (float) FNT_ifloor((*ypos + b->yoff) + 0.5f);
      q->x0 = x;
      q->y0 = y;
...
static int fnt__ray_intersect_bezier(float orig[2], float ray[2], float q0[2], float q1[2], float q2[2], float hits[2][2])
{
...
static int fnt__compute_crossings_x(float x, float y, int nverts, fnt_vertex *verts)
{
   int i;
...
static float fnt__cuberoot( float x )
{
   if (x<0)
...
// x^3 + a*x^2 + b*x + c = 0
static int fnt__solve_cubic(float a, float b, float c, float* r)
{
   float s = -a / 3;
   float p = b - a*a / 3;
   ...
   if (d >= 0) {
      ...
      float v = (float) FNT_acos(-FNT_sqrt(-27/p3) * q / 2) / 3; // p3 must be negative, since d is negative
      float m = (float) FNT_cos(v);
      float n = (float) FNT_cos(v-3.141592/2)*1.732050808f;
      r[0] = s + u * 2 * m;
      r[1] = s - u * (m + n);
      r[2] = s - u * (m - n);

      //FNT_assert( FNT_fabs(((r[0]+a)*r[0]+b)*r[0]+c) < 0.05f);  // these asserts may not be safe at all scales, though they're in bezier t parameter units so maybe?
      //FNT_assert( FNT_fabs(((r[1]+a)*r[1]+b)*r[1]+c) < 0.05f);
      //FNT_assert( FNT_fabs(((r[2]+a)*r[2]+b)*r[2]+c) < 0.05f);
      return 3;
   }
}
...
static unsigned char * fnt_GetGlyphSDF(const fnt_fontinfo *info, float scale, int glyph, int padding, unsigned char onedge_value, float pixel_dist_scale, int *width, int *height, int *xoff, int *yoff)
{
   ...
   fnt_GetGlyphBitmapBoxSubpixel(info, glyph, scale, scale, 0.0f,0.0f, &ix0,&iy0,&ix1,&iy1);
...

   {
      ...
      float *precompute;
      fnt_vertex *verts;
      int num_verts = fnt_GetGlyphShape(info, glyph, &verts);
      ...
      for (i=0,j=num_verts-1; i < num_verts; j=i++) {
         if (verts[i].type == FNT_vline) {
            ...
            float dist = (float) FNT_sqrt((x1-x0)*(x1-x0) + (y1-y0)*(y1-y0));
      ...
      for (y=iy0; y < iy1; ++y) {
         for (x=ix0; x < ix1; ++x) {
            ...
            int winding = fnt__compute_crossings_x(x_gspace, y_gspace, num_verts, verts); // @OPTIMIZE: this could just be a rasterization, but needs to be line vs. non-tesselated curves so a new path

            for (i=0; i < num_verts; ++i) {
               float x0 = verts[i].x*scale_x, y0 = verts[i].y*scale_y;

               if (verts[i].type == FNT_vline && precompute[i] != 0.0f) {
                  ...
                  dist = (float) FNT_fabs((x1-x0)*(y0-sy) - (y1-y0)*(x0-sx)) * precompute[i];
                  FNT_assert(i != 0);
                  if (dist < min_dist) {
                  ...
                  if (sx > box_x0-min_dist && sx < box_x1+min_dist && sy > box_y0-min_dist && sy < box_y1+min_dist) {
                     ...
                     if (a_inv == 0.0) { // if a_inv is 0, it's 2nd degree so use quadratic formula
                        ...
                        num = fnt__solve_cubic(b, c, d, ret);
                     }
...
{
   return fnt_GetGlyphSDF(info, scale, fnt_FindGlyphIndex(info, codepoint), padding, onedge_value, pixel_dist_scale, width, height, xoff, yoff);
}
...
static fnt_int32 fnt__CompareUTF8toUTF16_bigendian_prefix(fnt_uint8 *s1, fnt_int32 len1, fnt_uint8 *s2, fnt_int32 len2)
{
   fnt_int32 i=0;
...
static int fnt_CompareUTF8toUTF16_bigendian_internal(char *s1, int len1, char *s2, int len2)
{
   return len1 == fnt__CompareUTF8toUTF16_bigendian_prefix((fnt_uint8*) s1, len1, (fnt_uint8*) s2, len2);
}

// returns results in whatever encoding you request... but note that 2-byte encodings
// will be BIG-ENDIAN... use fnt_CompareUTF8toUTF16_bigendian() to compare
...
static int fnt__matchpair(fnt_uint8 *fc, fnt_uint32 nm, fnt_uint8 *name, fnt_int32 nlen, fnt_int32 target_id, fnt_int32 next_id)
{
   fnt_int32 i;
   ...
   for (i=0; i < count; ++i) {
      ...
      if (id == target_id) {
         // find the encoding
         fnt_int32 platform = ttUSHORT(fc+loc+0), encoding = ttUSHORT(fc+loc+2), language = ttUSHORT(fc+loc+4);
         ...
         if (platform == 0 || (platform == 3 && encoding == 1) || (platform == 3 && encoding == 10)) {
            ...
            fnt_int32 matchlen = fnt__CompareUTF8toUTF16_bigendian_prefix(name, nlen, fc+stringOffset+off,slen);
            if (matchlen >= 0) {
               ...
               if (i+1 < count && ttUSHORT(fc+loc+12+6) == next_id && ttUSHORT(fc+loc+12) == platform && ttUSHORT(fc+loc+12+2) == encoding && ttUSHORT(fc+loc+12+4) == language) {
                  ...
                  if (slen == 0) {
                     ...
                     ++matchlen;
                     if (fnt_CompareUTF8toUTF16_bigendian_internal((char*) (name+matchlen), nlen-matchlen, (char*)(fc+stringOffset+off),slen))
                        return 1;
                  }
               } else {
...
static int fnt__matches(fnt_uint8 *fc, fnt_uint32 offset, fnt_uint8 *name, fnt_int32 flags)
{
...
static int fnt_FindMatchingFont_internal(unsigned char *font_collection, char *name_utf8, fnt_int32 flags)
{
   fnt_int32 i;
   for (i=0;;++i) {
      fnt_int32 off = fnt_GetFontOffsetForIndex(font_collection, i);
      if (off < 0) return off;
      if (fnt__matches((fnt_uint8 *) font_collection, off, (fnt_uint8*) name_utf8, flags))
         return off;
   }
}

static int fnt_BakeFontBitmap(const unsigned char *data, int offset,
                                float pixel_height, unsigned char *pixels, int pw, int ph,
                                int first_char, int num_chars, fnt_bakedchar *chardata)
{
   return fnt_BakeFontBitmap_internal((unsigned char *) data, offset, pixel_height, pixels, pw, ph, first_char, num_chars, chardata);
}

static int fnt_GetFontOffsetForIndex(const unsigned char *data, int index)
{
   return fnt_GetFontOffsetForIndex_internal((unsigned char *) data, index);
}

static int fnt_GetNumberOfFonts(const unsigned char *data)
{
   return fnt_GetNumberOfFonts_internal((unsigned char *) data);
}

static int fnt_InitFont(fnt_fontinfo *info, const unsigned char *data, int offset)
{
   return fnt_InitFont_internal(info, (unsigned char *) data, offset);
}

static int fnt_FindMatchingFont(const unsigned char *fontdata, const char *name, int flags)
{
   return fnt_FindMatchingFont_internal((unsigned char *) fontdata, (char *) name, flags);
}

static int fnt_CompareUTF8toUTF16_bigendian(const char *s1, int len1, const char *s2, int len2)
{
   return fnt_CompareUTF8toUTF16_bigendian_internal((char *) s1, len1, (char *) s2, len2);
}
