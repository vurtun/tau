...
static void
ren__blit_img_slow(struct sys_ren_target *tar,
...
static void
ren__blit_img_fast(struct sys_ren_target *tar, const unsigned *src_img,
                   int x, int y, const struct sys_rect *sub_img, int stride,
                   const struct ren_clip *clip, unsigned col) {
#ifdef CPU_SIMD_256
  /* clip */
  if (col_a(col) == 0) return;
...
static void
ren__drw_cmd(struct ren *ren, struct sys_ren_target *tar,
