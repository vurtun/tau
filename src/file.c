...
static void
file_view_tree_node_del(struct file_tree_view *tree,
...
static void
file_tree_update(struct sys *sys) {
  assert(sys);
...
static void
file_view_tree_open(struct file_tree_view *tree, struct sys *sys, struct str p) {
  assert(tree);
  assert(sys);
...
static void
file_view_cd(struct sys *sys, struct str path) {
  assert(sys);
  struct scope scp = {0};
  struct arena *tmp = fs.tmp_arena;
  scope_begin(&scp, tmp);
...
static void
file_list_view_setup(struct sys *sys, struct gui_ctx *ctx) {
  assert(sys);
  assert(ctx);
  ...
  /* open home directory */
  file_view_cd(sys, fs.home);
}
static void
file_tree_view_setup(struct sys *sys) {
  assert(sys);
...
file_view_setup(struct sys *sys, struct gui_ctx *ctx, struct arena *mem,
  ...
  file_list_view_setup(sys, ctx);
  file_tree_view_setup(sys);
}
static void
file_view_update(struct sys *sys) {
  assert(sys);
...
static void
ui_file_lst_view_nav_bar(struct file_list_view *lst, struct gui_ctx *ctx,
                         struct gui_panel *pan, struct gui_panel *parent) {
  assert(lst);
  assert(ctx);
  assert(pan);
  ...
  gui.pan.begin(ctx, pan, parent);
  {
    ...
    if (gui.btn.ico(ctx, &home, pan, ICO_HOME)) {
      ...
      file_view_tree_open(&fs.tree, ctx->sys, fs.home);
      file_view_cd(ctx->sys, fs.home);
    }
    ...
    if (gui.btn.ico(ctx, &up, pan, ICO_ARROW_CIRCLE_UP)) {
      ...
      if (file.str - lst->full_path > fs.home.len) {
        file_view_cd(ctx->sys, strp(lst->full_path, file.str));
      }
    }
...
static void
ui_file_view_tbl(struct file_list_view *lst, struct gui_ctx *ctx,
                 struct gui_panel *pan, struct gui_panel *parent) {
  assert(lst);
  assert(ctx);
  assert(pan);
  ...
  if (chdir) {
    ...
    if (fi->isdir) {
      file_view_tree_open(&fs.tree, ctx->sys, fi->fullpath);
      file_view_cd(ctx->sys, fi->fullpath);
...
static void
ui_file_view_tree_key(struct file_tree_view *tree, struct gui_ctx *ctx,
                      struct gui_lst *lst) {
  assert(tree);
  assert(ctx);
  assert(lst);
...
static void
ui_file_view_tree(struct file_tree_view *tree, struct gui_ctx *ctx,
                  struct gui_panel *pan, struct gui_panel *parent) {
  assert(tree);
  assert(ctx);
  assert(pan);
  ...
  gui.pan.begin(ctx, pan, parent);
  {
    ...
    if (reg.lst.sel.mod) {
      ...
      struct file_tree_node *n = tree->lst[tree->sel];
      file_view_cd(ctx->sys, n->fullpath);
    }
    if (reg.lst.ctl.has_focus) {
      /* key handling */
      ui_file_view_tree_key(tree, ctx, &reg.lst);
    }
  }
  gui.pan.end(ctx, pan, parent);
}
...
static void
ui_file_sel_view(struct file_list_view *lst, struct gui_ctx *ctx,
                 struct gui_panel *pan, struct gui_panel *parent) {
  assert(lst);
  assert(ctx);
  assert(pan);
  ...
  gui.pan.begin(ctx, pan, parent);
  {
    ...
    case FILE_VIEW_LIST: {
      ...
      ui_file_view_tbl(lst, ctx, &tbl, pan);
    } break;
    case FILE_VIEW_MENU: {
...
static void
ui_file_sel_split(struct gui_ctx *ctx, struct gui_panel *pan,
                  struct gui_panel *parent) {
  assert(ctx);
  assert(pan);
  assert(parent);
  ...
  gui.pan.begin(ctx, pan, parent);
  {
    ...
    gui.splt.begin(ctx, &spt, pan, GUI_SPLIT_FIT, GUI_HORIZONTAL, lay, fs.split);
    {
      ...
      ui_file_view_tree(&fs.tree, ctx, &tree, &spt.pan);
      gui.splt.sep(ctx, &spt, lay, fs.split);

      struct gui_panel tbl = {.box = spt.item};
      ui_file_sel_view(&fs.lst, ctx, &tbl, &spt.pan);
    }
...
static int
ui_file_sel(dyn(char) *filepath, struct gui_ctx *ctx, struct gui_panel *pan,
            struct gui_panel *parent) {
  assert(ctx);
  assert(pan);
  assert(parent);
  ...
  gui.pan.begin(ctx, pan, parent);
  {
    ...
    ui_file_lst_view_nav_bar(&fs.lst, ctx, &nav, pan);
    ui_file_sel_split(ctx, &tbl, pan);
